...
- `Node.js backend`（**单进程**）负责连接 Godot agent WebSocket、action 投递、world event 入库、perception manifest 转发、HTTP / debug 接口，**并在同进程内**运行 agent runtime（AgentHost + two-track-agent session、调用 LLM 与工具并提交 action）
- `SQLite` 负责持久化；网关与 agent runtime 之间的 action / event / perception-manifest / status 总线走**进程内事件总线**（`plugins/message-bus.ts`），不再依赖 Redis 或第二个进程

> 历史说明：早期 backend 拆成 `gateway` + `worker` 两个进程，靠 Redis pub/sub 通信。单机 ~10 玩家场景不需要横向扩容，已合并为单进程 + 进程内 `MessageBus`（`setImmediate` 异步派发，保留 Redis 的 fire-and-forget 与错误隔离语义）。

```mermaid
flowchart LR
  Player["Player Input"] --> Client["Godot Client\nsrc/levels/town.gd\nUI + Camera + ENet Client"]
  Client <--> Runtime["Godot Runtime\nheadless authority\nCharacters + World + Sim"]
  Backend["Node Backend (single process)\nFastify + Godot WS client\n+ in-process MessageBus\n+ AgentHost / two-track-agent"] -->|action.submit / control| Runtime
...
- 启动 Fastify 应用
- 注册 SQLite、message bus、action bus、character status bus、agent runtime、Godot agent client 等插件
...
- 通过 `godot-message-handler.ts` 把 world/action 消息转成 DB 写入，把 perception manifest 发布到进程内 bus
...
- `action-log-service`
  ...
  - 通过进程内 action bus 通知 godot client 向 Godot 投递 `action.submit` / `action.cancel`
  - 记录 Godot `action.ack` 并供 Agent tool 等待 terminal result
- `world-event-bus`
  - 把 runtime world event 发布给 agent runtime 消费
- `perception-manifest-bus`
  - 把 Godot push 上来的 character perception manifest（id 清单）发到进程内 bus，agent runtime 收到后写 `AgentHostStateCache.manifestByCharacter`
...
- `character-status-bus`
  - 把 agent runtime 的 thinking 状态转发给 godot client 插件，再发给 Godot

### 5.3 Agent Runtime 层（同进程）

关键位置：

- [backend/src/plugins/agent-runtime.ts](../../backend/src/plugins/agent-runtime.ts)
...
- 作为 Fastify 插件在网关同进程内启动；订阅进程内 world event、character perception manifest、game time bus
...
- 通过 Agent tools 调用 `submitAction()`，再由 action bus / godot client 投递给 Godot

这意味着当前 AI 运行时不是直接控制 Godot，而是：

`world event -> manifest cache + sqlite SELECT -> agent runtime reasoning -> action_log -> action.submit -> Godot execute -> action.ack / world event`
...
- SQLite 是 Godot 和 backend 共享真值层：Godot 端 `db.gd` CREATE 全部 game-world 表（character_states / item_instances / farm_states / farm_plots / workstation_states / container_states / location_markers / shelf_listings / trade_offers / world_events / runtime_sessions / character_groups），持续 UPSERT；backend 只读不 CREATE。Backend 自有表：`action_log` / `agent_sessions` / `agent_session_messages` / `runtime_storage` / `runtime_characters` / `debug_settings`。Perception manifest 不落库（in-memory cache）
- 进程内 `MessageBus`（`plugins/message-bus.ts`）负责 action / world event / perception-manifest / game time / character status 各条 bus，连接网关与同进程 agent runtime（取代原 Redis pub/sub）
...
-> world event bus -> agent runtime
-> agent runtime 结合 character_context 推导 action tool
-> action_log + action bus
-> backend godot client 下发 action.submit
...
Godot 状态变更 -> 同步 UPSERT sqlite 真值
              ...
              -> 进程内 perception-manifest bus -> agent runtime 入 manifest cache
              -> 进程内 world-event bus -> agent runtime / AgentHost 路由到 two-track-agent session
...
| `backend/src/plugins` | SQLite、message bus、agent runtime、Godot agent client、action/status bus 装配 | Fastify glue |
...
backend 不直接改世界，它主要负责“连接 Godot、记录事件/快照/action、在同进程内驱动 agent runtime”。Agent 负责解释意图，但最终仍只能通过 action 让 Godot 执行。
...
11. [backend/src/plugins/agent-runtime.ts](../../backend/src/plugins/agent-runtime.ts)
