...
| Session loop（turn 调度 / abort / queueing） | **per-agent** | two-track 有自己的 release/abort/fresh-context 调度；未来 agent 可能完全不同 |
...
| `game-time.ts` | `gameTimeTotalMinutes / eventGameMinuteValue / gameTimeFromEventData` |
| `agent-message.ts` | `userMessage / isAssistantMessage / agentMessageRole / assistantUsage / usageTokenCount / usageCostUsd` |
| `log-format.ts` | `sanitizeForLog / extractToolCalls / formatContentText / snapshotAgentTools` + 敏感 key 屏蔽 |
