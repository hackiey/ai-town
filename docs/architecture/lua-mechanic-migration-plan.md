...
`affect.*` 已有（async，lua 走完 GDScript apply）：`stamina` / `hunger` / `hp` / `broadcast_speech` / `crop_state` / `farm_state` / `crop_destroy` / `give_item` / `add_condition` / `remove_condition` / `set_alive` / `world_event`。
...
|---|---|---|---|
| Character 生理（hunger 衰减、starving 扣血、hungry condition）| character.gd:259-307 | ~80 | `modify_hp`, `remove_condition` |
...
✅ Step 3.5  affect.add_condition / world_event / /cast 入口
✅ Step 4  Character 生理 → physiology.lua  (affect.hp / remove_condition / set_alive；smoke 10/10)
...
**作用域**：`character.gd::apply_slow_tick` + `_refresh_hungry_condition` + alive 翻转。

**新增 affect**（Q2：lua 声明，GDScript setter 善后）：
- `affect.hp(target, amount)` — 同 `affect.hunger`/`affect.stamina` 命名（不叫 `modify_hp`，保持现有命名风格）
- `affect.remove_condition(target, condition_id)` — 解除 condition（hungry 阈值清除走这条）
...
**调用方**：
- `apply_slow_tick` → `MechanicHost.invoke("physiology", "on_slow_tick", ctx)` + 收尾（_expire_timed_conditions / _sync_head_status / tick_spoilage / _persist_state）
- `refresh_conditions` → `MechanicHost.invoke("physiology", "on_hunger_changed", ctx)` + sync_head + persist
...
**遗留**：`add_condition` effect 应用末尾会自动 `target.refresh_conditions()`，对非 hungry 的 condition 是冗余触发；Step 6.4 sleeping 迁 lua 时一并解耦。
...
4. **睡眠 start/commit** — `add_condition("sleeping", -1)` + 唤醒时 `remove_condition` + `modify_hp`；不需要 `set_sleeping` 专用 affect
...
|---|---|---|
| 4 | ✅ `hp`, `remove_condition`, `set_alive` | 低 |
...
| 6.4 | `set_sleeping` 用 `add_condition("sleeping", -1)` 已够；hp 回复用 `modify_hp` | 低 |
...
- **优先扩展现有**：能用 `add_condition("sleeping", -1)` + setter 副作用表达的，不再加 `set_sleeping`
...
6. **Backend condition / event 翻译表** — `CONDITION_PROMPTS` / `EVENT_PROMPTS` 把 lua id 转自然语言塞进 LLM context
...
9. **Lua 自定义持久化字段（mechanic_state）**（Q5 决议推迟）— 未来"NPC 仇恨值 / 派系声望 / 法术 concentration"等用例需要，方案是给 character_states 加 `mechanic_state TEXT` JSON 列 + `affect.set_mechanic_field` / `world.get_mechanic_field` API。Step 4–6 全部用现有字段 + `active_conditions` 完成，不需要这条；等真有 per-actor 持久 lua 字段需求时立项
...
| Q5 | lua 自定义持久化字段（"魔法学派偏好"等）？ | **本计划不做，移到 §6**。Step 4–6 全部用现有字段 + `active_conditions` 完成 | 实际盘点 9 个 sub-step 全用现有 schema 够；mechanic_state JSON 列等真有需求再立项 |
...
- 2026-05-15：Step 4 ✅。physiology.lua + `affect.hp` / `remove_condition` / `set_alive` 落地；apply_slow_tick / refresh_conditions 改走 lua hook；Character.alive 加 setter + `_on_alive_changed` 虚 hook；`scripts/physiology_smoke.tscn` 10 场景全 PASS。`affect.modify_hp` 改名 `affect.hp` 与现有命名一致。
