...
- NPC AI 核心（two-track action + working memory + planning）
- 服务端权威 + 存档 + 反作弊
- 玩家账号 / 计费 / 镇分配
...
| 数据层 | SQLite 作为当前主库；网关与同进程 agent runtime 之间走进程内 MessageBus（action / event / snapshot / status），无 Redis |
...
4. **服务器托管栈细化**：Godot runtime 部署、Node 服务部署、SQLite 备份与观测方案
...
本节记录当前“Agent runtime（同进程）→ backend gateway → Godot runtime → action ack → Agent tool result”的主链路和仍需补强的技术债。
...
- 玩家自然语言指令走 `player.command` WebSocket 消息。Gateway 只记录为 `player_command` world event 并发布给同进程 agent runtime；runtime 找到对应 player agent session，主动查询 Godot live snapshot 后解析命令并调用已有 action tools。

### 7.2 Character 指令执行

- 当前 runtime 支持 `move_to_location`。agent runtime 会把当前 `visibleLocations` 中的具体地点名或 alias 写进 `location` 参数 enum，例如 `blacksmith_shop`、`铁匠铺`、`current_location`，不接收坐标；隐藏子地点需要先前往父地点后才会出现在参数 enum 里。
- 当前 runtime 支持 `say_to`、`sleep`、`use_item`、`pick_up_item`、`drop_item`、`offer`、`respond`、`create_item`、货架、容器、王室消耗、领工资、工作台和农事 action。`say_to` 会由 Godot 计算 `affectedCharacterIds` 并广播头顶气泡。真实库存、交易、创造物落地属于 Godot server 权威逻辑，agent runtime 只消费 server 回写的 snapshot/event。
...
- action 提交路径是：Agent tool -> `submitAction()` -> SQLite `action_log` -> 进程内 `action bus` -> godot client `action.submit` -> Godot `BackendActionRunner`。工具调用会轮询 `action_log` 等待 terminal ack，再把结果回填给 LLM。
...
- SQLite 已作为后端主存接入：保存 `action_log`、`world_events`、`runtime_storage`、`agent_sessions`、`agent_session_messages` 等；网关与同进程 agent runtime 之间走进程内 MessageBus（world event、snapshot request/response、game time、action、character status），不再有 Redis。
- `world.event` 已接入说话、物品、交易、货架、工作台、农事、容器、王室消耗、领工资等链路。战斗等事件类型仍是未来工作。
- agent runtime 在 event/think 前发送 `character.snapshot.request`；runtime 返回 `character.snapshot`，包含 `currentLocationId`、`characterAttributes`、`nearbyBuildings.near/far`、`nearbyCharacters.near/far`、`nearbyItems.near/far`、`nearbyFarms`、`nearbyWorkstations`、`nearbyShelves`、`nearbyContainers`、`inventory`、`backpack`、`pendingTrades`。backend 不持久化快照，agent runtime 只把本次 live snapshot 用于当前 prompt / event classification；空区块不出现在 prompt 中。
- agent runtime 不推导交易状态，只信 Godot server 写入 context 的 `pendingTrades`。`respond(kind:"trade")` 只在 context 存在 pending trade 时使用，平时交易相关 tool 只有 `offer`。
- Character memory、session message 持久化和 two-track planning 已接入。Thinking 轨定时、significant event、think-first 和 timeline backlog 触发 working memory 更新；Action 轨工具失败由下一轮 action prompt 自纠错。
...
- 真实集成测试仍应沉淀成脚本化测试：启动 backend（单进程）、headless Godot server，再用 `action.request` / debug agent 覆盖 action 生命周期、cancel、重连 replay 和 snapshot 更新。
...
- v1.0 (2026-06-04)：删除 Redis，backend 由 gateway + worker 两进程合并为单进程；五条 bus 改走进程内 `MessageBus`（`plugins/message-bus.ts`，setImmediate 异步派发），worker 逻辑并入 `plugins/agent-runtime.ts`，debug agent-run-filter 迁到 SQLite `debug_settings` 表；`./scripts/dev all` 现一键起 Godot server + backend
