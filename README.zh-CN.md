...
- **双轨 NPC 大脑。** 每个 NPC 同时跑一条快速反应轨（数秒内行动）和一条慢速策略轨（反思、规划、从失败中恢复）——既保持灵敏，又目标明确，而代价远低于"每步都推理"。（详见下文。）
...
- 一条 **strategic track（策略轨）** —— 开启 extended reasoning —— 定期对处境进行反思、规划，并写下一段简短的 working memory 简报（记忆 / 反思 / 规划，思路上接近 Generative Agents 的 "Smallville" 论文）。
